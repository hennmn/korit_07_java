...
/*
    ...
    scores  배열에 점수를 입력할 수 있도록 할겁니다.
    1 번 학생의 이름을 등록하세요 >>> 4.5
    2 번 학생의 이름을 등록하세요 >>> 4.4
    3 번 학생의 이름을 등록하세요 >>> 4.3
    4 번 학생의 이름을 등록하세요 >>> 4.2
    5 번 학생의 이름을 등록하세요 >>> 4.1
...
public class Array02 {
    public static void main(String[] args) {
        ...
        human = scanner.nextInt();  // nextInt() 때문에 다음 nextLine의 엔터가 적용되어 바로 2번으로 넘어갈 가능성 -> 베리어 필요 **
        scanner.nextLine();  // 이 베리어를 깔거나  **

        // humans라고 하는 배열의 크기와 scores라는 배열의 크기는 동일해야 합니다.
        ...
        for(int i = 0; i < humans.length; i++) {        // humans.length나 scores.length나 human이나 다 같죠
            ...
            humans[i] = scanner.nextLine();  // 이걸 scanner.next로 바꾸면 됨 **
        }

//       // 이건 모든 direction을 반영한 코드라고 할 수 있겠습니다. 그리고 밑에는 그냥 실행 예만 반영한 형태도 볼게요.
...
//        System.out.println("학생들의 점수 총합은 " + sum + "점 입니다.");
        ...
        // 증값값을 += 2를 통해서 강제 조정할 경우
        // index는 0부터 시작인데 굳이 1로 지정해줘야하기 때문에 오히려
        // 조건물이 있는 것보다 가독성이 떨어지고 잘못 해석할 확률이 높습니다.
        ...
        for(int i = 0; i < humans.length; i++) {
            if(i % 2 == 1) {
                System.out.println(humans[i] + " 학생의 점수는 " + scores[i]);
            }
        }
       // 와 같이 작성하는 편입니다.
