...
public class FactoryMain {
    public static void main(String[] args) {
        ...
        Factory factory1 = new Factory() {      // 명시하진 않았지만 Factory 클래스를 상속 받았다고 할 수 있음
            @Override
...
//                System.out.println(this.getName() + "에서 [" + model + " ] 을 생산합니다.");
        ...
        };  // ;를 꼭 명시해야 하는데 이 전체 코드라인이 메서드를 정의하는 것이 아니라 객체를 생성한 것이기 때문
        ...
        /*
             ...
             재사용하지 않고 한 번만, 즉 이 경우에는 FactoryMain에서만 쓰고 치울 에정이면 사용하기 좋겠습니다.

             사실 우리는 교육 상황이기 때문에 모든 클래스를 한 번만 정의하고 그걸로 객체 한 두개 만들고 치우는 경우가 많아서
             모든 수업을 익명 클래스로 처리할 수도 있겠죠.
         */
        PhoneFactory phoneFactory1 = new PhoneFactory();
        phoneFactory1.setName("애플 스마트폰 공장");

        // 부모 클래스에서 선언한 추상 클래스를 자식 클래스에서 구현부 작성한 대로 작동 되는지 확인
        ...
        /*
            ...
            부모클래스와 자식 클래스의 메서드가 '다르게' 굴러갈 경우에만 이루어지면 되고,
            부모 클래스의 메서드를 동일하게 사용할거라면 애초에 받아올 필요도 없이 그냥 쓰면 됩니다.

            다만 여러분이 자식 클래스를 정의했을 때 부모 클래스에 특정 메서드가 있음을 명확하게 알고 있어야
            할 필요가 있죠.

            부모 클래스에서  showInfo() 라는 메서드가 있는 걸 몰랐다면 여러분들은
