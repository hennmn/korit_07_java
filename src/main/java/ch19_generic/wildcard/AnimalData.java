...
public class AnimalData<T> {
    ...
    /*
        ...
        Tiger / Human 객체를 필드에 대입할 겁니다.

        근데 T에는 어떤 클래스도 들어갈 수 있기 때문에 현재 상황에서는
        Car클래스의 객체도 animal 필드에 들어갈 수 있을 겁니다.

        저희의 이번 목표는
        특정 클래스라면(즉 Animal 클래서의 상곡을 받은 서브클래스라면)
    ...
    public void showData() {
        ...
        // 여기서는 의문의 여지가 없이 호랑이가 네 발로 뜁니다./ 살마이 두 발로 뜁니다가 출력되겠네요.

        if(animal.getClass() == Human.class) {
            ((Human)animal).read();     // 현재 필드에 들어간 animal 객체의 클래스를 확인하여
            // (.getClass()를 통해서)  그것이 Human 클래스면
