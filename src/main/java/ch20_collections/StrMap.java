...
//        koreanScores1.put("김육", 100.0);
        ...
        // 특정 key의 value 수정
        // #1. 아까 말한 것처럼 key에는 value가 하나 밖에 없기 때문에 최근 걸로 덮어쓰기 됩니다.
        ...
        /*
            ...
            있는 key 라면 value에 재대입 하게 됩니다.

             반면 .replace()는 없는 key라면 value값의 갱신이 일어나지 않습니다.
...
//        List<Map.Entry<String, String>> entryList2 = strMap1.entrySet();
        ...
        // Mqp -> List로 바로 가는 게 가능한지 여부를 따질 필요가 있습니다.
...
//        Set<String> valueSet = strMap1.values(); 이게 불가능한 이유는 뭘까요?
        /*
            이상에서 강제 형변환을 하지 않은 상태에서 오류가 발생하는 이유는 key는 어차피 중복을 하용하지 않기 때문에
            Set으로 바꾸는 게 가능한 반면,
            복수의  key에서 동일한 value가 존재할 수 있기 때문에 직접적으로 Set으로 바꾸지 않습ㄴ디ㅏ.
